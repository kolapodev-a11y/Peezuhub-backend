 ...
 *
 * Email behaviour
 *   1. Admin alert  → ADMIN_EMAILS (peezutech@gmail.com) when payment verified
 *   2. Buyer receipt → user.email when premium activated for the FIRST time
 */
...
// ─── Constants ────────────────────────────────────────────────────────────────
const PREMIUM_PRICE_KOBO    = 500_000;   // ₦5,000 in kobo
...
async function syncAllUserListingsPremium(userId, premiumExpiresAt, reference) {
  ...
  // Surface only available + approved listings as featured/verified
  await Listing.updateMany(
    { user: userId, saleStatus: 'available', status: 'approved' },
    { $set: { isFeatured: true, isVerified: true } },
  );

  // Hide sold / unapproved listings from featured position
  await Listing.updateMany(
...
router.get('/paystack/verify/:reference', auth, async (req, res, next) => {
  try {
    ...
    const response = await axios.get(
      ...
      { headers: { Authorization: `Bearer ${paystackSecret}` }, timeout: 10_000 },
    );
    ...
    if (payment.status === 'success') {
      // Detect whether we already processed this exact reference
      ...
      const activatedAt = alreadyActivated && user.premiumActivatedAt
        ? new Date(user.premiumActivatedAt)
        : new Date();

      const expiresAt = alreadyActivated && user.premiumExpiresAt
        ? new Date(user.premiumExpiresAt)
        : new Date(Date.now() + PREMIUM_DURATION_DAYS * 24 * 60 * 60 * 1_000);

      // Persist premium state on user
      user.premiumPlan        = 'seller_premium';
      user.premiumStatus      = 'active';
      ...
      // ── Send emails only on first activation ──────────────────────────────
      if (!alreadyActivated) {
        // 1️⃣  Admin alert → peezutech@gmail.com
        ...
        // 2️⃣  Buyer receipt → the member who paid
